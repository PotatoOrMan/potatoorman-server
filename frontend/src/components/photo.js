...
function CapturedImage({ capturedImage, potatoIdx }) {
  ...
  const fetchDatas = async (capturedImage) => {
    try {
      ...
      const response = await fetch(`https://13.125.230.135:8081/savephotoApi/save`, {
        method: 'POST',
        body: formData
      });
  ...
  const fetchData = async (capturedImage) => {
    try {
      ...
      // const rr = await fetch('https://localhost:8081/savephotoApi/save', {
      //   method: "POST"
      // })
      // console.log(rr.text())
      ...
      // 이메일 전송 요청
      const emailResponse = await fetch(`https://13.125.230.135:8081/savephotoApi/submit`, {
        method: 'POST',
        headers: {
